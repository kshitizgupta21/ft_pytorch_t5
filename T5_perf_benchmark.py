...
def translate(args_dict):
    ...
    load_from_bin = args_dict["load_from_bin"]
    if load_from_bin:
        ## read checkpoint config if exists
        ckpt_path = args_dict['ckpt_path'] + f"/{tensor_para_size}-gpu/"
        ...
        t5_model = T5ForConditionalGeneration.from_pretrained(model_path, low_cpu_mem_usage=True, torch_dtype=torch.float16)
    ...
    if time_args.find("1") != -1:
        ...
        if rank == 0 and not skip_gemm:
            ...
            cmd = f"./FasterTransformer/build/bin/t5_gemm {batch_size} {beam_size} {input_seq_len} " \
                ...
                f"{decoder_config.vocab_size} {is_fp16} {tensor_para_size} {is_append}"
    ...
    if time_args.find("3") != -1:
        ...
        if rank == 0 and not skip_gemm:
            is_fp16 = 1 if args_dict['data_type'] == 'fp16' else 0
            cmd = f"./FasterTransformer/build/bin/t5_gemm {math.ceil(batch_size / pipeline_para_size)} {1} {input_seq_len} " \
                f"{encoder_config.d_model} {encoder_config.num_heads} {encoder_config.d_kv} {encoder_config.d_ff} " \
                f"{decoder_config.d_model} {decoder_config.num_heads} {decoder_config.d_kv} {decoder_config.d_ff} " \
                f"{decoder_config.vocab_size} {is_fp16} {tensor_para_size} {is_append}"
            print(f"Run gemm test: {cmd}")
    ...
    if load_from_bin:
        ft_encoder_weight.load_from_bin(ckpt_path, "Megatron")
        ft_decoding_weight.load_from_bin(ckpt_path, "Megatron")
    else:
    ...
    for i in range(len(translation_result_list)):
        ...
        tmp_time_list = []
        while iter_idx < min_iterations or (datetime.now() - begin_time).total_seconds() < min_duration:
            iter_idx += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lib_path', '--lib_path', type=str, default="FasterTransformer/build/lib/libth_transformer.so", metavar='STRING',
                        help='the path of FasterTransformer pytorch t5 op library.')
    parser.add_argument('--load_from_bin', action="store_true", help="Whether to load from FT ckpt or HF model")
    ...
    parser.add_argument('-model_path', '--model_path', type=str, default=None, metavar='STRING',
                        help='T5 model path.')
    parser.add_argument('-model', '--model', type=str, metavar='STRING',
                        help='T5 model size. Only used when --model_path=None and --ckpt_path=None', choices=["t5-small", "t5-base", "t5-large", "t5-3b", "t5-11b"])
    ...
    parser.add_argument('-skip_gemm', '--skip_gemm', action="store_true",
                        help='Skip the gemm autotuning by not calling the ./FasterTransformer/build/bin/t5_gemm binary.')
